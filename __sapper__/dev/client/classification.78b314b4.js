...
// (42:4) {#if isProfessionalEligible}
function create_if_block_1(ctx) {
	var p, t0, t1, button, t2, button_disabled_value, dispose;
	...
	return {
		...
		l: function claim(nodes) {
			...
			button = claim_element(nodes, "BUTTON", { disabled: true }, false);
			var button_nodes = children(button);

			t2 = claim_text(button_nodes, "I request to be classified as a setProfessional Client.");
			button_nodes.forEach(detach);
			this.h();
		},

		h: function hydrate() {
			add_location(p, file$2, 42, 4, 620);
			button.disabled = button_disabled_value = !ctx.isProfessionalEligible;
			add_location(button, file$2, 43, 6, 644);
			dispose = listen(button, "click", setProfessional);
		},
		...
		p: function update(changed, ctx) {
			if ((changed.isProfessionalEligible) && button_disabled_value !== (button_disabled_value = !ctx.isProfessionalEligible)) {
...
// (46:4) {#if isProfessionalEligible}
function create_if_block(ctx) {
	...
	return {
		c: function create() {
			button = element("button");
			t = text("I confirm i'm a private Client.");
			this.h();
		},
		...
		l: function claim(nodes) {
			...
			t = claim_text(button_nodes, "I confirm i'm a private Client.");
			button_nodes.forEach(detach);
			this.h();
		},

		h: function hydrate() {
			add_location(button, file$2, 46, 7, 829);
			dispose = listen(button, "click", next);
		},
...
function create_fragment$2(ctx) {
	var div, t0, label0, input0, t1, t2, label1, input1, t3, t4, label2, input2, t5, t6, t7, dispose;

	var if_block0 = (ctx.isProfessionalEligible) && create_if_block_1(ctx);

	var if_block1 = (ctx.isProfessionalEligible) && create_if_block();
	...
	return {
		...
		l: function claim(nodes) {
			...
			label2 = claim_element(div_nodes, "LABEL", {}, false);
			var label2_nodes = children(label2);

			input2 = claim_element(label2_nodes, "INPUT", { type: true, value: true }, false);
			var input2_nodes = children(input2);

			input2_nodes.forEach(detach);
			t5 = claim_text(label2_nodes, "\n\tNone of the above");
		...
		h: function hydrate() {
			...
			add_location(input0, file$2, 26, 1, 344);
			add_location(label0, file$2, 25, 0, 335);
			ctx.$$binding_groups[0].push(input1);
			attr(input1, "type", "radio");
			input1.__value = 2;
			input1.value = input1.__value;
			add_location(input1, file$2, 31, 1, 423);
			add_location(label1, file$2, 30, 0, 414);
			ctx.$$binding_groups[0].push(input2);
			attr(input2, "type", "radio");
			input2.__value = 0;
			input2.value = input2.__value;
			add_location(input2, file$2, 36, 1, 503);
			add_location(label2, file$2, 35, 0, 494);
			add_location(div, file$2, 22, 1, 317);
		...
		m: function mount(target, anchor) {
			...
			input2.checked = input2.__value === ctx.option;
		...
		p: function update(changed, ctx) {
			...
			if (changed.option) input2.checked = input2.__value === ctx.option;
...
// (28:4) {#if $lsFin.clientType === "Private Client" && $lsFin.option===undefined}
...
function create_fragment$3(ctx) {
	var div, h3, t0, t1, p0, t2, t3, p1, t4, t5_value = ctx.$lsFin.clientType, t5, t6, p2, t7, t8_value = ctx.$lsFin.option, t8, t9, t10, current;

	var if_block0 = (ctx.$lsFin.clientType === undefined) && create_if_block_1$1();

	var if_block1 = (ctx.$lsFin.clientType === "Private Client" && ctx.$lsFin.option===undefined) && create_if_block$1();
	...
	return {
		...
		p: function update(changed, ctx) {
			...
			if ((!current || changed.$lsFin) && t8_value !== (t8_value = ctx.$lsFin.option)) {
			...
			if (ctx.$lsFin.clientType === "Private Client" && ctx.$lsFin.option===undefined) {
				if (!if_block1) {
